...
router.post('/edited', (req,res)=>{
        var db = require('../../lib/database')();
        db.query(`UPDATE \`vehicles\` SET maker=?, model=?, year=?, platenumber=?, state=? WHERE id=?`,[req.body.maker, req.body.model, req.body.year, req.body.platenumber, req.body.state, Number(req.body.id)], (err, rows) => {
            if (err) console.log(err);
            res.redirect('/users/%60'+ req.body.id+'%60')
        });
});
router.post('/delete', (req,res) => {
    var db = require('../../lib/database')();
    db.query(`DELETE from vehicles WHERE id = ?`,[req.body.id], (err, rows) => {
        if (err) console.log(err);
        res.redirect('/index');
    });
});
